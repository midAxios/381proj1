...
architecture structural of sra32 is
...
begin
    HIGH: if i = 31 generate
        M: mux2t1 port map(s0(i), iA(31), iShamt(0), s1(i));  -- MSB fill with sign
    end generate;
...
begin
    HIGH: if i >= 30 generate
        M: mux2t1 port map(s1(i), iA(31), iShamt(1), s2(i));  -- MSB fill
    end generate;
...
begin
    HIGH: if i >= 28 generate
        M: mux2t1 port map(s2(i), iA(31), iShamt(2), s3(i));
    end generate;
...
begin
    HIGH: if i >= 24 generate
        M: mux2t1 port map(s3(i), iA(31), iShamt(3), s4(i));
    end generate;
...
begin
    HIGH: if i >= 16 generate
        M: mux2t1 port map(s4(i), iA(31), iShamt(4), s5(i));
    end generate;
